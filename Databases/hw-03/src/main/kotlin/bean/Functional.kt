...
class Functional {
    ...
    fun removeUseless(functionals: List<Functional>, stringBuilder: java.lang.StringBuilder): Functional? {
        for (trying in from) {
            val newFrom = from.toMutableSet().apply { remove(trying) }
            val newFunctional = Functional(newFrom, to)
            val newFunctionals = Functionals(functionals.toMutableList())
            val closureTask = ClosureTask(newFrom)
            val closure = closureTask.getRealClosure(newFunctionals)
            if (closure.containsAll(to)) {
                stringBuilder.appendLine("Найдено новое бесполезное: ${toString(from, to)} -> ${toString(newFrom, to)}")
                from.remove(trying)
