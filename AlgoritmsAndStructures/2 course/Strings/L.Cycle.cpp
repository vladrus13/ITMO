...
pair <int32*, vector<int32> > suffix_array(string const& s) {
...
int main() {
    ...
    int32 k;
    cin >> s >> k;
    pair <int32*, vector <int32> > templater = suffix_array(s);
    auto suff = templater.first;
    auto uniq = templater.second;
    if (uniq[s.size() - 1] + 1 < k) {
        cout << "IMPOSSIBLE";
        return 0;
    }
    int32 temp = suff[lower_bound(uniq.begin(), uniq.begin() + s.size(), k - 1) - uniq.begin()];
    cout << s.substr(temp, s.size() - temp) << s.substr(0, temp);
    re
}
