...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
            ...
            val minAccount : AcquiredAccount? = acquire(minIndex, this)
            val maxAccount : AcquiredAccount? = acquire(maxIndex, this)
            if (minAccount != null && maxAccount != null) {
                val fromAccount = if (minIndex == fromIndex) minAccount else maxAccount
                val toAccount = if (minIndex == toIndex) minAccount else maxAccount
