...
class HttpServer {
    ...
    Map <File, Integer> hashing = new HashMap<>();
    ...
    private Integer hash(File a) throws IOException {
        Integer hash = (int) a.lastModified();
        if (a.isDirectory()) return 0;
        byte[] body = readFile(a);
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        if (request.headers.containsKey("If-None-Match")) {
            byte[] body;
            if (hashing.containsKey(file)) {
                if (hash(file).toString().equals(request.getHeader("If-None-Match"))) {
            ...
            responseSetter(response, body, file, getContentType(file));
            hashing.put(file, hash(file));
            return;
        }
        if (file.isDirectory()) {
            if (request.getUri().equals("/")) {
                ...
                String ans = "static_index";
                byte[] body = ans.getBytes();
                response.setStatusCode(200);
                responseSetter(response, body, file, "text/html");
                return;
            }
        }
        if (file.isFile()) {
    ...
    private void responseSetter(Response response, byte[] body, File file, String contentType) throws IOException {
        response.setBody(body);
        response.setHeader("Content-Length", Integer.toString(body.length));
        response.setHeader("Content-Type", contentType);
        response.setHeader("ETag", hash(file).toString());
    }
