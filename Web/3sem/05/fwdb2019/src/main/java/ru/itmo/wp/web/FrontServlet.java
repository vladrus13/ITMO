...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        String[] entryes = {route.getAction(), "before", "after"};
        boolean allEntryesIsFound = false;
        for (Class<?> clazz = pageClass; !allEntryesIsFound && clazz != null; clazz = clazz.getSuperclass()) {
            allEntryesIsFound = true;
            Method[] methodsClass = clazz.getDeclaredMethods();
            for (Method it : methodsClass) {
                for (String entry : entryes) {
                    if (Objects.equals(it.getName(), entry)) {
                        methods.put(entry, it);
                    }
                }
            }
            for (String entry : entryes) {
                if (!methods.containsKey(entry)) {
                    allEntryesIsFound = false;
                    break;
                }
            }
        }
        ...
        try {
            for (Method method1 : methods.values()) {
                method1.setAccessible(true);putUser(request, view);
            }
            Class[] types = method.getParameterTypes();
            Object[] fillingTypes = new Object[types.length];
            for (int i = 0; i < types.length; i++) {
                if (types[i] == HttpServletRequest.class) {
                    fillingTypes[i] = request;
                }
                if (types[i] == Map.class) {
                    fillingTypes[i] = view;
                }
            }
            methods.get("before").invoke(page, request, view);
            method.invoke(page, fillingTypes);
            methods.get("after").invoke(page, request, view);
