...
<header>
    ...
    <div class="enter-or-register-box">
        <#if logged_user_id??>
            <@userlink user=findBy(users, "id", logged_user_id)[1] nameOnly=true/>
            |
            <a href="#">Logout</a>
        <#else>
            <a href="#">Enter</a>
...
<#macro handlelink user nameOnly>
    <#if nameOnly>
        <a href="/user?handle=${user.handle}">${user.handle}</a>
    <#else>
        <a href="/user?handle=${user.handle}" style="font-weight: bold; color:${user.color}; text-decoration: none">${user.handle}</a>
    </#if>
</#macro>
        ...
        <#if view>
            <#if countSymbols!=0>
                <#if countSymbols<line?length>
                    <p>${line?substring(0, countSymbols-1)}...</p>
