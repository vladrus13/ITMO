...
public class functionLexer {
     ...
     * @param text text
     */
    public functionLexer(String text) {
        this.text = text;
        currentChar = 0;
    }
     ...
     */
    public functionToken getCurrentToken() {
        return currentToken;
    }
     ...
     */
    public void nextToken() throws function.exception.ParseException {
        skipSpaces();
        if (text.length() == currentChar) {
            currentChar++;
            currentToken = new functionToken(functionEnum.End, null);
            return;
        }
        if (text.length() < currentChar) {
            throw new function.exception.ParseException("Get a token after end of data");
        }
        functionEnum token = null;
        Matcher matcher = null;
        for (functionEnum terminal : functionEnum.values()) {
        ...
        if (token == null) {
            throw new function.exception.UnsupportedCharacterException("Can't get character '"+ text.substring(currentChar, currentChar + 1) + "' at > " + getProblem() + " <");
        }
        ...
        currentToken = new functionToken(token, text.substring(start, currentChar));
    }
}