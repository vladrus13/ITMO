...
public class BinarySearchMissing {
    ...
    private static int iterativeBinarySearch(int x) {
        ...
        if (a.get(r) <= x) {
            return (a.get(r) == x ? r: -r - 1);
        }
        // inv: answer not exist
        return -a.size() - 1;
    }

    // pre: (0 <= l <= r < a.size & a_l >= a_(l + 1)) & X ... (a_(r - 1) >= a_r) & x in Z & !(0 <= l <= r < a.size)
    ...
    private static int recursiveBinarySearch(int l, int r, int x) {
        ...
        if (r - l < 2) {
            ...
            if (a.get(l) <= x) return (a.get(l) == x ? l: -l - 1);
            // pre: a_r <= x
            // post: N = a_i <= x & (any N j < i & a_i > x) & i in [l, r]
            if (a.get(r) <= x) return (a.get(r) == x ? r: -r - 1);
            // pre: a_l > x & a_r > x & x in a[l..r] -> not exist i, satisfying the requirements. Return -a.size() - 1
            return -a.size() - 1;
        }
    ...
    public static void main(String[] argm) {
        ...
        if (a.size() == 0) {
            ...
            System.out.println((MODE == 0 ? iterativeBinarySearch(x) : recursiveBinarySearch(0, a.size() - 1, x)));
            a.clear();
            x = 0;
        }
    }
